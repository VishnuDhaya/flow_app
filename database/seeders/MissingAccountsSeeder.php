...
class MissingAccountsSeeder extends Seeder
{
    /**
	...
	private function import_missing_account($borrower_data){
		...
		$data['acc_prvdr_name'] = "EzeeMoney";
		$data['acc_prvdr_code'] = "UEZM";
		$data['type'] = "Wallet";
    ...
    public function run(){
		try{
			...
			$missing_data_prvdr_datas = DB::select("select data_prvdr_cust_id,cust_id,created_by,created_at, owner_person_id from borrowers where cust_id not in (select cust_id from accounts where cust_id is not null) and data_prvdr_code = 'UEZM'");
