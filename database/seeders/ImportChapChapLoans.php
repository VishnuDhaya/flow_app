...
class ExcelImport implements ToCollection, WithCalculatedFormulas{
	...
	private function import_record($row_id, $xl_record,$prods_data){
		...
		try{
			...
			DB::beginTransaction();
			session()->put('data_prvdr_code', 'CCA');
			...
			if(!array_key_exists('account_id', $record_obj)){
				...
				thrw("Account not configured TEST");
			}
	
			$max_loan_amount = $xl_record[7];
			$flow_fee = $xl_record[8];
			$comments = $xl_record[24];
			$txn_id = $xl_record[17];
			$re_txn_id = $xl_record[19];
			...
			$full_comments = "$status | $comments | $txn_id  | $re_txn_id";
	...
	private function get_prods_data(){
		...
		$prods_data = DB::select("select id,product_code,duration,product_type,max_loan_amount,flow_fee,flow_fee_type from loan_products where data_prvdr_code='CCA'");
		return $prods_data;
	}
	...
	private function disburse_record($record_obj,$loan, $xl_record){
		...
		$due_date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($xl_record[11]);
		$disbursal_date = $disbursal_date->format('Y-m-d');
		...
		$disbursal_req['from_ac_id'] = $this->lender_accounts ;
		$disbursal_req['to_ac_id'] = $record_obj['account_id'];
		$disbursal_req['txn_id'] = $xl_record[17];
	...
	private function repay_record($record_obj,$loan, $xl_record,$row_id,$comments){
		...
		$loan = $loan['loan'];
		$payment_req = array();
		$paid_date = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($xl_record[14]);
		$paid_date = $paid_date->format('Y-m-d');
		$payment_req['amount'] = $xl_record[9]; 
		$payment_req['loan_doc_id'] = $loan->loan_doc_id;
		$bank_name = trim($xl_record[23]);
		$to_ac_id = $this->lender_accounts;
		$payment_req['to_ac_id'] = $to_ac_id;
		$payment_req['txn_date'] = $paid_date;
		$payment_req['txn_id'] = $xl_record[19];
		...
		$status = $xl_record[18];
		$payment_req['repay_comm'] = 0;
		
		if ($status == 'Paid back late'){
			if ($xl_record[16] == ""){
				$payment_req['penalty_collected'] = 0;	
			}else{
			$payment_req['penalty_collected'] = $xl_record[16];
