...
def wait_for_visibility(by, token, wait_sec = 30):
...
def find_if_exists_by_xpath(xpath, time=30):
    try:
        wait_for_visibility(By.XPATH, xpath, time)
...
def find_if_exists_by_id(id,time =30):
    try:
        wait_for_visibility(By.ID, id,time)
        return driver.find_element_by_id(id)
    except (NoSuchElementException, TimeoutException) as err:
        return None

def find_if_exists_by_name(name, time=30):
    try:
        wait_for_visibility(By.NAME, name, time)
